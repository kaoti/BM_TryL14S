...
void Server::HandleInput()
{
	...
	while (true)
	{
		...
		{
			if (General::processParameter(userinput, "con"))
			{
			...
			else if (General::processParameter(userinput, "broadall"))		//broadcasts commands to all clients
			{
				General::outputMsg("Entering broadcast mode. To disable, type 'exitSession'", 1);
				currentSessionID = -2;
			}
			else if (General::processParameter(userinput, "ls"))	//counts clients (TODO: list clients)
			{
				...
				General::outputMsg("Please connect to a session with 'con'", 2);
		}
		...
		else						
		{
			...
			else if (General::processParameter(userinput, "sw"))
			{
				inputInt = atoi(userinput.c_str());
				int tempInt = connections.size() - 1;
				if (inputInt > tempInt)
			...
			else if (userinput.find("rc") != std::string::npos)
			{
